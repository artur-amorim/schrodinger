...
class Base_Interp{
    ...
    public:
		...
		int getCor() const;
		int getDJ() const ;
		const vector<T>* getXX() const;
		const vector<T>* getYY() const;
...
class Spline_Interp:public Base_Interp<T> {
	...
	public:
		Spline_Interp();
		Spline_Interp(const vector<T> &X, const vector<T> &Y, T yp1 = 1e99, T ypn = 1e99);
		vector<T> getY2() const;
		void sety2(const vector<T> &X, const vector<T> &Y, T yp1 = 1e99, T ypn = 1e99);
...
template<class T>
Base_Interp<T>::Base_Interp(const vector<T> &X, const vector<T> &Y, const int mm)
{
	n = X.size() ;
	m = mm ;
	jsav = 0 ;
...
template<class T>
int Base_Interp<T>::getN() const {return n;}

template<class T>
int Base_Interp<T>::getM() const {return m;}

template<class T>
int Base_Interp<T>::getJSav() const {return jsav;}

template<class T>
int Base_Interp<T>::getCor() const {return cor;}

template<class T>
int Base_Interp<T>::getDJ() const {return dj;}

template<class T>
const vector<T>* Base_Interp<T>::getXX() const {return XX;}

template<class T>
const vector<T>* Base_Interp<T>::getYY() const {return YY;}
...
template<class T>
void Base_Interp<T>::copy(const Base_Interp<T> &interp)
{
	n    = interp.getN();
	m    = interp.getM();
	jsav = interp.getJSav();
...
template<class T>
Base_Interp<T>& Base_Interp<T>::operator= (const Base_Interp<T> &spline)
{
...
template<class T>
Spline_Interp<T>::Spline_Interp() : Base_Interp<T>() {}

template<class T>
Spline_Interp<T>::Spline_Interp(const vector<T> &X, const vector<T> &Y, T yp1, T ypn) : Base_Interp<T>(X, Y, 2)
{
	Y2.resize(X.size()) ;
...
template<class T>
vector<T> Spline_Interp<T>::getY2() const {return Y2;}

template<class T>
void Spline_Interp<T>::sety2(const vector<T> &X, const vector<T> &Y, T yp1, T ypn)
{
	T p, qn, sig, un ;
	int n = Y2.size() ;
	vector<T> U(n-1) ;
